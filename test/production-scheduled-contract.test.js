...
test('the production smoke adapter uses a real browser and retains its report', async () => {
  ...
  // The fallback globe and live bundle activate sequentially; neither wait may
  // consume the other phase's application-sized allowance.
  assert.match(source, /waitForProductionClient\(page, READY_TIMEOUT_MS\)/);
  const readiness = await readFile(new URL('../src/production-client-ready.js', import.meta.url), 'utf8');
  assert.match(readiness, /waitForSelector[\s\S]*timeout: timeoutMs/);
  assert.match(readiness, /waitForFunction[\s\S]*timeout: timeoutMs/);
