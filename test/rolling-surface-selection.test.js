...
test('selects current quality-approved MODIS and VIIRS surface observations in deterministic priority order', () => {
  const selected = selectRollingSurfaceObservations({
    ...
    candidates: [
      ...
      candidate({ href: './stale-regression.npz', validAt: '2026-08-01T12:00:00Z', observedFrom: '2026-07-25T00:00:00Z', observedTo: '2026-08-01T23:59:59Z', producedAt: '2026-08-02T06:00:00Z' }),
...
test('accepts a delayed observation after a newer publication while suppressing an already-contributed window', () => {
  const delayed = candidate({ validAt: '2026-08-23T12:00:00Z', observedFrom: '2026-08-16T00:00:00Z', observedTo: '2026-08-23T23:59:59Z' });
  const duplicate = candidate({ href: './duplicate-copy.npz' });
  const selected = selectRollingSurfaceObservations({
    targetTime: '2026-08-28T12:00:00Z',
    maxCandidateAgeDays: 16,
    minAcceptedFraction: 0.25,
    previousObservationWindows: [{
      ...
      observedFrom: duplicate.observedFrom,
      observedTo: duplicate.observedTo,
    }],
    candidates: [duplicate, delayed],
  });

  assert.deepEqual(selected.map(item => item.href), ['./mcd43a4-2026-08-24.npz']);
});
