...
- The first bundled Earth state preserves the existing 5.4K NASA Blue Marble surface, high-resolution night lights, elevated cloud shell and its interpreted 25 August 2026 MODIS density state, lunar map, 16K Milky Way, and Hipparcos catalogue as a deterministic static fallback. The browser no longer retrieves or interprets scientific satellite products itself; subsequent Earth-state production can replace these layers without changing the renderer's interface.
