...
An optional `byteLength` and SHA-256 `checksum` protect each candidate. Every NPZ is a prepared north-up EPSG:4326 grid matching the seasonal surface dimensions and contains `reflectance` (`H×W×3`, linear 0–1), `land`, `quality`, `cloud`, `cloud_shadow`, `haze`, `geometry_quality`, and `age_days`. Provider-specific decoding and QA-bit interpretation remain in the adapters; the compositor sees one stable scientific interface.
