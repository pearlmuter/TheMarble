...
export function auroraRaySegments(origin, ray) {
  ...
  const outer = interval(AURORA_OUTER_RADIUS), ground = interval(1), inner = interval(AURORA_INNER_RADIUS);
...
vec4 auroraSegments(vec3 origin,vec3 ray) {
  vec2 outer=auroraSphereInterval(origin,ray,AURORA_OUTER);
  ...
  vec2 inner=auroraSphereInterval(origin,ray,AURORA_INNER);
